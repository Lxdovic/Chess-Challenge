...
public class TestBot : IChessBot
{
    ...
    private ulong _desiredPosition = 0xffff0000000000;
    private bool _color;
    

    private enum MoveFlag { Alpha, Exact, Beta }
    private Move[] _killerMoves = new Move[128];
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        for (int currentDepth = 1; currentDepth <= 100; currentDepth++)
        {
            ...
            Console.WriteLine("depth {0,1} time {1,5}, pv {2}{3} score {4}",
                currentDepth,
    ...
    private int Negamax(int alpha, int beta, int depth, int ply)
    {
        ...
        if (depth == 0) return _color == _board.IsWhiteToMove ? Evaluate(_board) : Quiesce(alpha, beta, 2);
        ...
        _transpositionTable[positionKey % 0x7FFFFF] = new TranspositionTableEntry(positionKey, bestScore, depth, flag, bestMove);
    ...
    {
        return BitboardHelper.GetNumberOfSetBits(board.AllPiecesBitboard & _desiredPosition) 
               + BitboardHelper.GetNumberOfSetBits(_color ? board.WhitePiecesBitboard : board.BlackPiecesBitboard);
    }
